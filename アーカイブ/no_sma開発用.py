...
class MyModel(object):
    ...
    def on_enter_created_new_arrow(self):
        ...
        if self.name == "Session_1360":
            print("ここじゃ")
            print(self.start_index)
            print(self.new_arrow_index)
    ...
    def on_enter_has_position(self):
        entry_slice = self.full_data[self.entry_index:self.next_next_new_arrow_index]
        ...
        #トレード結果の算出
        has_position_slice = self.full_data[self.entry_index:self.next_new_arrow_index]
        self.close()
    ...
    def check_no_SMA(self,index, neck_price):
        ...
        if self.up_trend:
            if neck_price > max(sma_value) - 0.005:
                return True
            else:
                return False
        else:
            if self.full_data[index,3] < min(sma_value) + 0.005:
                return True
            else:
                return False
    ...
    def calculate_trade_result(self, session, data_slice):
        """
        エントリー後のトレード結果を判定してログを返す関数。

        session: MyModelのインスタンス
        data_slice: エントリー後からnext_next_new_arrowまでのスライス(np.ndarray)

        data_slice は少なくとも以下のカラムを持つ前提：
        0: time
        2: high
        3: low
        4: close
        ...
        # TP・SLの判定用にboolean配列を作成
        if session.up_trend:
            # 上昇トレンドの場合、take_profitを上に超えたら利確、SLは下回ると損切り
        ...
        # 最初に到達したインデックスを取得（到達しなければ inf）
        tp_index = np.argmax(tp_hit) if np.any(tp_hit) else np.inf
        sl_highlow_index = None
        sl_sml_index = None
        
        if session.up_trend:
            sl_highlow_hit = lows <= session.highlow_stop_loss
            sl_sml_hit = lows <= session.sml_stop_loss
            sl_highlow_index = np.argmax(sl_highlow_hit) if sl_highlow_hit.any() else np.inf
            sl_sml_index = np.argmax(sl_sml_hit) if sl_sml_hit.any() else np.inf
        else:
            sl_highlow_index = np.argmax(sl_highlow_hit) if sl_highlow_hit.any() else np.inf
            sl_sml_index = np.argmax(sl_sml_hit) if sl_sml_hit.any() else np.inf
        ...
        first_event_index = min(np.argmax(tp_hit) if tp_hit.any() else np.inf, min_sl_index)

        # 結果判定
        if first_event_index == np.inf:
            # TPもSLも到達しなかった場合（最後の足で強制決済）
            exit_index = -1
            close_price = data_slice[-1, 4]
            result = "force_close"
        elif first_event_index == np.argmax(tp_hit):
            # TP到達
            ...
            if sl_highlow_index < sl_sml_index:
                ...
                exit_index = sl_highlow_index
                exit_price = session.highlow_stop_loss
            else:
                exit_reason = "S/L (sml)"
                exit_index = sl_sml_index
                exit_price = session.sml_stop_loss
            result = "loss"

        # 該当なしなら強制クローズ処理
        if first_event_index == np.inf:
            exit_price = data_slice[-1, 4]
        ...
        trade_log = {
            ...
            "exit_time": pd.to_datetime(data_slice[exit_index, 0], unit="ns", utc=True),
            ...
            "result": result,
            "profit_loss": exit_price - session.entry_line if session.up_trend else session.entry_line - exit_price,
...
class WaveManager(object):
    ...
    def analyze_sessions(self):
        ...
        if self.trade_logs:
            ...
            # entry_time列が文字列やfloatになっている可能性があるので、日時に変換する
...
def merge_arr(base_arr, sml_arr, additional_sma_df):
    ...
    # final_merged.to_csv("check_df.csv")
...
def process_data(conditions):
    ...
    fromdate = conditions.get("fromdate", datetime(2024, 11, 17, 20, 0, tzinfo=pytz.UTC))
...
if __name__ == "__main__":
    conditions = {
        "symbol": "USDJPY",
        "fromdate": datetime(2015, 2, 21, 0, 0, tzinfo=pytz.UTC), #始まる日時
