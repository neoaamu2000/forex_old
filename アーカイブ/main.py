...
#-------------ネックラインに関する制作-----------------
    ...
    def handle_created_new_arrow(self, df, sml_df):
        ...
        print(f"{self.name}、37ライン：{self.base_fibo37}、開始pvt：{self.start_pivot}、newぴぼと：{self.new_arrow_pivot}")
...
#--------------------ここから------------------------------
    ...
    def handle_infibos(self, df, sml_df):
        ...
        if len(self.determined_neck) > 0: 
            print("ここでテスト",self.determined_neck)
            self.neck_determine()
    ...
    def record_state_time(self):
        ...
        self.state_times[state_name] = last_pivot_data
    ...
    def on_enter_created_new_arrow(self):
        ...
        print(f"{self.name}: Entered 'created_new_arrow' state.、ニューアローピボット : {self.new_arrow_pivot}")
...
#---------------------------------------------------------------------
    ...
    def get_sml_pivots_after_goldencross(self,sml_pivots):
        ...
        """
        print(f"ここでテスト！セッション名:{self.name},トレンド：{self.up_trend},開始：{self.pivot_data}、ポテンシャル：{self.potential_neck}、ゴールデンクロス,{self.time_of_goldencross}、ステート：{self.state}")
    ...
    def check_potential_to_determine_neck(self): 
        ...
        if self.potential_neck:
            ...
            print(f"ここでテスト！セッション名:{self.name},トレンド：{self.up_trend},開始：{self.pivot_data}、ポテンシャル：{self.potential_neck}")
            if self.up_trend is True:
...
def process_data(symbol="USDJPY"):
    ...
    df = original_df.iloc[:7000].copy()
    sml_df = original_df.iloc[:7000].copy()
...
# 初期のSMA計算（この時点では200本分）
    ...
    pivot_data = detect_pivots(df.copy(), POINT_THRESHOLD=0.01, LOOKBACK_BARS=15, name = "BASE_SMA",arrow_spacing=10)
    ...
    for idx in range(7000, len(original_df)):
    ...
    # 新しいローソク足データ（1行）を取得
        ...
        # 3. 20MAのピボット検出
        # --- ピボット検出の更新（直近ウィンドウのみ） ---
        new_pivot = update_detect_pivot(df, point_threshold=0.01, lookback_bars=15, consecutive_bars=3, arrow_spacing=10, name = "BASE_SMA")
