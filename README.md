...
このツールで使ってる手法や、このREADMEで説明している要素をｷﾞｭｯｯｯっとした画像を作りました。為替取引したことがあって目が良い方なら、1分眺めてるとツールとページの内容を大体把握できると思います。
...
１・11/23 現時点でこのREADMEはGeminiに書いてもらった状態です。見た感じ正しくない内容がちょこちょこあります。12月中旬頃には技術選定の意図もさらに載せて、詳しく正確なREADMEを公開できるようにします。

２・もしツールを動かしたい場合、csvを証券会社からダウンロードして、pklファイル変換が必要です。このレポジトリは発想まとめみたいな意味合いで公開しています。


このプロジェクトは、外国為替（FX）市場における特定の取引戦略の有効性を検証するためのバックテストツール、および将来的なリアルタイム取引への拡張を目的とした自動売買システムです。
検証開始から検証完了まで全てターミナル上で操作します。
（manage_data.pyを実行するだけです。UI操作でパラメーター設定などできれば楽だなと思いつつ、バックエンドで追加するべき機能が多すぎて多すぎてまだ手を付けられてません）
...
## 高速化のための発想転換：MQL5の常識を超えるアーキテクチャ

このツールの真価は、単にPythonで書かれていることではありません。MQL5のバックテストが持つ「ローソク足を1本ずつ順番に処理する」という常識を覆し、検証時間を劇的に短縮するための、いくつかの「発想の転換」にあります。

### 思想①：「計算」と「分析」の完全分離 - "準備が9割"

MQL5のEAでは、バックテストを開始するたびに、`OnInit`や`OnCalculate`内でインジケータ（移動平均線など）の値を毎回計算するのが一般的です。しかし、過去のデータに対するインジケータの値は、何度計算しても同じです。

そこでこのツールでは、**「計算」と「分析」のフェーズを完全に分離しました。**

1.  **計算フェーズ（事前準備）**:
    -   まず、`manage_data.py`のようなスクリプトを使い、元のCSVデータから必要な全てのテクニ��ル指標（20MA, 75MAなど）を**事前に計算**します。
    -   そして、ローソク足の価格と、計算済みのインジケータ値を全て含んだ巨大なデータセットを、高速に読み書きできる`Pickle`形式のファイルとして保存します。

2.  **分析フェーズ（バックテスト実行）**:
    -   `main.py`や`main_parallels.py`が実行するバックテストでは、この**計算済みのPickleファイルを読み込むだけ**です。
    -   これにより、テスト実行時にインジケータを計算する時間はゼロになり、純粋なロジックの検証だけにCPUリソースを集中させることができます。これは、特に複数のインジケータを使う複雑なロジックほど、絶大な効果を発揮します。

### 思想②：「1本ずつの処理」から「範囲での処理」へ - "木を見ず森を見る"

MQL5のバックテストは、`OnTick`や`OnCalculate`といったイベント駆動モデルであり、価格が動くたびに「ローソク足1本分」のデータで処理を行います。これは直感的ですが、膨大な過���データを処理するには非効率です。

このツールでは、`NumPy`の力を借りて、この**逐次処理の呪縛**から脱却しました。

例えば、「エントリー後に損切りラインに到達したか？」を判定する場面を考えてみましょう。

-   **MQL5的な発想（1本ずつチェック）**:
    ```python
    # forループで1本ずつ価格をチェックする（低速）
    for i in range(len(data_slice)):
        if data_slice[i, 'low'] <= stop_loss_price:
            exit_index = i
            break 
...
-   **本ツールでの発想（範囲で一括チェック）**:
    ```python
    # 期間全体の安値配列と損切りラインを一度に比較（超高速）
    sl_hit_conditions = data_slice[:, 3] <= stop_loss_price 

    # 条件に合致した最初の足のインデックスを瞬時に取得
    sl_hit_indices = np.where(sl_hit_conditions)[0]
    exit_index = sl_hit_indices[0] if sl_hit_indices.size > 0 else -1
    ```

これはほんの一例です。ピボットの検出、フィボナッチゾーンへの到達判定など、ロジックの���ぼ全てが、このような「範囲（配列）」を対象とした一括処理（ベクトル演算）で実装されています。これにより、Pythonのループ処理が持つ遅さを完全に克服し、C言語に匹敵する速度で条件判定を行っています。

### 思想③：「直列処理」から「並列処理」へ - "タイムマシンによる分業"

このツールの高速化を決定づけるのが、`main_parallels.py`に実装された並列処理アーキテクチャです。これは、単に処理を分割するだけでなく、バックテストの「時間」という概念そのものを再定義します。

1.  **テスト期間の分割**:
    -   まず、検証したい長期間（例: 10年間）を、オーバーラップさせながら複数の短い期間（例: 8つの2年期間）に分割します。オーバーラップさせるのは、期間の境界でトレード機会を逃さないためです。

2.  **並列世界の創造と実行**:
    -   Pythonの`ProcessPoolExecutor`を使い、PCのCPUコア数に応じてプロセスを起動します（例: 8コアなら8プロセス）。
    -   分割された各期間を、それぞれのプロセスに割り当て、**完全に独立した世界のバックテストとして同時に実行**させます。あたかも、10年間の歴史を8人の分身が異なる時代から同時に検証するようなものです。

3.  **結果の統合と重複排除**:
    -   全ての並列世界でのバックテストが完了すると、それぞれの世界からトレードログ（結果）が集められます。
    -   最後に、これらのログを一つに結合し、オーバーラップ期間で発生したであろう重複トレードを`entry_time`を基準に除去します。これにより、あたかも単一のタイムラインでバックテストを実行したかのような、正確な最終結果が完成します。

この「期間分割による並列処理」は、ローソク足を最初から最後まで順番に処理する必要があるMQL5のストラテジーテスターでは**原理的に実現不可能**なアプローチであり、このツールが年単位のバックテストをわずか数分で完了できる最大の理由です。
