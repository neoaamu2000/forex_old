...
def pre_data_process(df,conditions)
...
def process_data(conditions):
    ...
    # 初期データとして先頭1600行を利用
    df = pd.read_pickle("currency_data/USDJPY/USDJPY_1M.pkl")

    # SMAの計算（BASE_SMA と SML_SMA）
    df = calculate_sma(df, window=[BASE_SMA,SML_SMA], name=["BASE_SMA","SML_SMA"])

    determine_trend(df, ["BASE_SMA","SML_SMA"])

    pivot_data = detect_pivots(df.copy(), POINT_THRESHOLD=BASE_threshold, LOOKBACK_BARS=BASE_lookback,
                                name="BASE_SMA", arrow_spacing=BASE_arrow_spacing)
    sml_pivot_data = detect_pivots(sml_df.copy(), POINT_THRESHOLD=SML_threshold, LOOKBACK_BARS=SML_lookback,
                                   consecutive_bars=SML_consecutive, name="SML_SMA", arrow_spacing=SML_arrow_spacing)
...
if __name__ == "__main__":
    conditions = {
        "symbol": "USDJPY",
        "fromdate": datetime(2023, 6, 19, 0, 0), #始まる日時
        "todate": datetime(2025, 2, 20, 0, 0), #終わる日時
