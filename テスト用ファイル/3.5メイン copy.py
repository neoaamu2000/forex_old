...
class MyModel(object):
    ...
    def handle_created_base_arrow(self):
        pivot_arr = self.full_data[:,9]
        valid = ~np.isnan(pivot_arr)
    # 有効な要素のインデックスを取得
        valid_indices = np.where(valid)[0]
    # self.start_index 以降の有効なインデックスだけ抽出して、最初の要素を選ぶ
        if np.any(valid_indices >= self.start_index):
            self.new_arrow_index = valid_indices[valid_indices >= self.start_index][0]
        else:
    ...
    def handle_created_new_arrow(self, df, sml_df):
        self.highlow_since_new_arrow = self.get_high_and_low_in_term(self.new_arrow_index)
...
#---------------------------------------------------------------------
    ...
    def get_high_and_low_in_term(self, time):
        """
...
class WaveManager(object):
    ...
    def add_session(self, start_index, prev_index, up_trend):
        ...
        if up_trend == "True":
            prev_price, start_price = self.full_data[prev_index,2], self.full_data[start_index,3]
            _,_,session.fibo_minus_20,session.fibo_minus_200, = detect_extension_reversal(prev_price, start_price,None, None, 0.2, 2)
        if up_trend == "False":
            prev_price, start_price = self.full_data[prev_index,3], self.full_data[start_index,2]
            session.fibo_minus_20, session.fibo_minus_200,_,_ = detect_extension_reversal(prev_price, start_price,-0.2,-2,None, None)
        return session
...
def process_data(conditions):
    ...
    # print(wm.full_data)
    # print(wm.full_data.shape)
    column_list = final_df.columns.tolist()
    # print(column_list)
    # print(np.datetime_as_string(wm.full_data[-100:,0].astype("datetime64[ns]"), unit='s'))
...
if __name__ == "__main__":
    conditions = {
        "symbol": "USDJPY",
        "fromdate": datetime(2025, 2, 16, 0, 0, tzinfo=pytz.UTC), #始まる日時
