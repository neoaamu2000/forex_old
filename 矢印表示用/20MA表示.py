...
def main(consecutive_bars=3):
    ...
    # ===== 5. トレンド転換の検出 =====
    POINT_THRESHOLD = 0.009  # 1ポイント以上のSMA差
    LOOKBACK_BARS = 15   # 過去5本内をチェック
    ...
    for i in range(3, len(df)):
        ...
        # ----- 上昇→下降（高値形成）の検出 -----
        if consecutive_down and up_trend == True:
            pivot_index = i
            # if pivot_index - last_pivot_index < 2:
            #     continue
        ...
        # ----- 下降→上昇（安値形成）の検出 -----
        if consecutive_up and up_trend == False:
            pivot_index = i
            # if pivot_index - last_pivot_index < 2:
            #     continue
            if pivot_index - LOOKBACK_BARS < 0:
